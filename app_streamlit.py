...
st.caption("No external libs. Sliders update on rerun. Designed to avoid curly-brace parsing issues.")
...
bg_color = "#0b1220" if dark else "#f9fafb"
ink_color = "#e5e7eb" if dark else "#0f172a"
show_vectors_js = "true" if show_vectors else "false"
...
<body>
  <div class="wrap">
    ...
    <div class="legend">
      Flocking (alignment/cohesion/separation) + Perlin flow field.
      Seed: __SEED__. Boids: __BOIDS__.
    </div>
  </div>
...
const SETTINGS = {
  BOIDS: __BOIDS__,
  TRAIL: __TRAIL__,
  NOISE_SCALE: __NOISE_SCALE__,
  FLOW_STRENGTH: __FLOW_STRENGTH__,
  SHOW_VECTORS: __SHOW_VECTORS__,
  DARK: __DARK__,
  SEED: __SEED__
};
...
function loop(){
  ...
  // avoid JS template literals to keep things simple
  const fillDark = 'rgba(2,4,8,' + (1-SETTINGS.TRAIL) + ')';
  const fillLight = 'rgba(250,250,255,' + (1-SETTINGS.TRAIL) + ')';
  ctx.fillStyle = SETTINGS.DARK ? fillDark : fillLight;
...
# Substitute placeholders without touching braces by using simple replace
html = (html
        .replace("__BG__", bg_color)
        .replace("__INK__", ink_color)
        .replace("__SEED__", str(seed))
        .replace("__BOIDS__", str(boids))
        .replace("__TRAIL__", str(trail))
        .replace("__NOISE_SCALE__", str(noise_scale))
        .replace("__FLOW_STRENGTH__", str(flow_strength))
        .replace("__SHOW_VECTORS__", "true" if show_vectors else "false")
        .replace("__DARK__", "true" if dark else "false")
        )
